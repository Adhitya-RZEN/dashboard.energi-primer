      ...
      {rows.map((row) => (
        ...
        >
          ...
          <strong className="text-sm text-slate-950">
            ...
            <small className="font-normal text-slate-500">ton</small>
          </strong>
        </div>
      ))}
    </div>
...
function TargetPanel({ data }: { data: OverviewData }) {
  if (!data.target)
    return (
      <OverviewUnavailable note="Target dan realisasi biomassa Laravel berasal dari Google Sheets row 56/59 kolom CO. Tabel kpi_targets PostgreSQL hanya berisi SFC dan heat rate, bukan target biomassa." />
    );
    ...
    <div>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-slate-600">
            Target Biomassa Tahun {data.query.year}
          </p>
        ...
        <span className="rounded-full bg-violet-50 px-2.5 py-1.5 text-[10px] font-bold text-violet-700">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        ...
        <div>
          ...
          <strong className="text-sm text-slate-950">
            {formatNumber(target.cumulative)}{" "}
            <small className="font-normal text-slate-500">ton</small>
          </strong>
        </div>
        <div>
          <span className="block text-[10px] text-slate-500">Sisa Target</span>
          <strong className="text-sm text-slate-950">
            {formatNumber(target.remaining)}{" "}
            <small className="font-normal text-slate-500">ton</small>
          </strong>
        </div>
      </div>
...
function HopPanel({ data }: { data: OverviewData }) {
  if (!data.hop)
    return (
      <OverviewUnavailable note="HOP Unit 1–3 hanya tersedia sebagai kolom AJ/AK/AL Google Sheets; PostgreSQL existing tidak menyimpan HOP per unit." />
    );
  return (
    <div>
      <div className="grid grid-cols-[1fr_.8fr_1fr] gap-3 border-b border-slate-200 pb-2 text-[10px] font-bold uppercase tracking-wide text-slate-400">
        <span>Unit</span>
      ...
      {data.hop.map((row) => (
        <div
          key={row.unit}
          className="grid grid-cols-[1fr_.8fr_1fr] items-center gap-3 border-b border-slate-100 py-3 last:border-0 last:pb-0"
        >
          <strong className="text-sm text-slate-950">{row.unit}</strong>
          <span className="text-right text-sm font-bold text-slate-800">
            {formatNumber(row.value, 1)}{" "}
            <small className="font-normal text-slate-500">hari</small>
          </span>
          <span
            className={`text-right text-xs font-bold ${row.status === "danger" ? "text-red-700" : row.status === "warning" ? "text-amber-700" : "text-emerald-700"}`}
          >
            <i
              className={`mr-1 inline-block size-1.5 rounded-full ${row.status === "danger" ? "bg-red-500" : row.status === "warning" ? "bg-amber-500" : "bg-emerald-500"}`}
            />
            {row.label}
          </span>
        </div>
    ...
    <div className="mx-auto w-full max-w-7xl space-y-8">
      ...
      <section aria-labelledby="summary-title" className="space-y-4">
        <DashboardSectionHeading
          ...
          description="Indikator utama pada periode terpilih."
        />
      ...
      <section aria-labelledby="consumption-title" className="space-y-4">
        <DashboardSectionHeading
          themeKey="overview"
          eyebrow="Tren harian"
          title="Konsumsi Energi Primer"
          description={`Perbandingan pemakaian harian · ${period.monthLabel}.`}
        />
        <DashboardChartPanel
          title="Konsumsi Energi Primer Harian"
          description="Batubara dan biomassa mengikuti source aktif."
        >
          <EnergyConsumptionChart series={data.series} />
        </DashboardChartPanel>
      </section>

      <section
        aria-label="Target dan status operasional"
        className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]"
      >
        <div className="space-y-4">
          <DashboardSectionHeading
            themeKey="target"
            eyebrow="Kinerja tahunan"
            title="Target & Realisasi"
          />
          <DashboardPanel>
            <TargetPanel data={data} />
          </DashboardPanel>
        </div>
        <div className="space-y-4">
          <DashboardSectionHeading
            themeKey="stok"
            eyebrow="Kesiapan operasi"
            title="Status Operasional (HOP)"
          />
          <DashboardPanel>
            <HopPanel data={data} />
          </DashboardPanel>
      ...
      <section aria-labelledby="detail-title" className="space-y-4">
        <DashboardSectionHeading
          ...
          description="Rincian pemakaian harian dan penerimaan bahan bakar."
        />
        <div className="grid gap-4 sm:grid-cols-2">
          <DashboardPanel>
            ...
            <p className="mt-1 text-xs text-slate-500">{focusLabel}</p>
            <div className="mt-4">
              <UnitValues
                rows={data.biomassDaily}
                emptyNote="Tidak ada data biomassa di source aktif."
              />
            </div>
          </DashboardPanel>
          <DashboardPanel>
            ...
            <div className="mt-4">
              <UnitValues
                rows={data.coalDaily}
                emptyNote="Tidak ada data konsumsi pada tanggal fokus."
              />
            </div>
          </DashboardPanel>
          <DashboardPanel>
            <h3 className="text-sm font-bold text-slate-950">
              Pemakaian Solar Harian
            </h3>
            <p className="mt-1 text-xs text-slate-500">{focusLabel}</p>
            <div className="mt-4">
              {metrics.solarConsumptionDaily.available ? (
                <div className="text-3xl font-extrabold text-slate-950">
                  {formatNumber(metrics.solarConsumptionDaily.value)}{" "}
                  <small className="text-xs font-normal text-slate-500">
                    liter
                  </small>
                </div>
              ) : (
                <OverviewUnavailable note="Tidak ada data solar di PostgreSQL existing." />
              )}
            </div>
          </DashboardPanel>
          <DashboardPanel>
            ...
            <div className="mt-4 text-3xl font-extrabold text-slate-950">
              {formatNumber(metrics.coalReceiptMonthly.value)}{" "}
              <small className="text-xs font-normal text-slate-500">ton</small>
            </div>
            <p className="mt-2 text-xs leading-5 text-slate-500">
              {metrics.coalReceiptMonthly.note}
            </p>
          </DashboardPanel>
        </div>
      </section>
