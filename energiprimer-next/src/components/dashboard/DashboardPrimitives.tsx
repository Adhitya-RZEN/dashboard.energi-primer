...
export function DashboardSectionHeading({
  ...
  return (
    <div>
      <p
        className={`text-[11px] font-bold uppercase tracking-[0.18em] ${theme.text}`}
      >
        {eyebrow}
      </p>
...
export function DashboardPanel({
  ...
  return (
    <div
      className={`rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)] sm:p-5 ${className}`}
    >
      {children}
    </div>
  );
...
export function DashboardDataStatus({ data }: { data: OverviewData }) {
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-2xl border border-blue-100 bg-blue-50/70 px-4 py-3 text-xs text-blue-950 sm:px-5">
      <span>
        <strong>Sumber aktif:</strong> {data.source.label}
        {data.source.worksheetEquivalent
          ? ` · ${data.source.worksheetEquivalent}`
          : ""}
      </span>
...
export function DashboardPageHeader({
  ...
  return (
    <header>
      ...
      <p
        className={`text-[11px] font-bold uppercase tracking-[0.2em] ${theme.text}`}
      >
        {theme.eyebrow}
      </p>
