...
export function OverviewKpiCard({
  ...
  const card = (
    <article
      className={`flex min-h-40 flex-col rounded-2xl border border-t-4 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)] transition hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(15,23,42,0.08)] ${colors.accent}`}
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          aria-hidden="true"
          className={`flex size-9 shrink-0 items-center justify-center rounded-xl text-xs font-bold ${colors.icon}`}
        >
          {metric.available ? "●" : "–"}
        </span>
      </div>
      <div
        className={`mt-auto flex items-baseline gap-2 pt-6 ${metric.available ? "" : "text-slate-400"}`}
      >
