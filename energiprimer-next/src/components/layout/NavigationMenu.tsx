...
function NavigationIcon({ name }: { name: NavigationIconName }) {
  ...
  switch (name) {
    case "overview":
      return (
        <svg aria-hidden="true" className="size-5 shrink-0" viewBox="0 0 24 24" {...commonProps}>
  ...
  return (
    <nav aria-label="Primary navigation">
      {navigationSections.map((section, sectionIndex) => (
        <div key={section.label} className={sectionIndex > 0 ? "mt-5 border-t border-slate-200 pt-4" : ""}>
          <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">
            {section.label}
          </p>
          <ul className="space-y-1">
            {section.items.map((item) => {
              const isActive = Boolean(item.href && (pendingHref === item.href || (item.href === "/dashboard" ? pathname === item.href : pathname === item.href || pathname.startsWith(`${item.href}/`))));
              ...
              return (
                <li key={item.label}>
                  {item.available && item.href ? (
                    ...
                    >
                      {isActive ? <span aria-hidden="true" className={`absolute left-0 h-5 w-0.5 rounded-r ${feedbackTheme.solid}`} /> : null}
                    ...
                    <span aria-disabled="true" className={className} title="Belum dimigrasikan">
                      <NavigationIcon name={item.icon} />
