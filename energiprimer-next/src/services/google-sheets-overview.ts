...
async function readSemanticWorksheet(worksheet: string): Promise<DynamicParserResult | null> {
  try {
...
async function readSheet(month: number, year: number): Promise<{
  worksheet: string;
  rows: SheetRow[];
...
function semanticMetricOrLegacy(
  ...
  const semanticMetric = overviewMetricFromResolved(resolved, unit, worksheet, label);
...
function semanticBiomassReceiptMetric(semantic: DynamicParserResult | null, worksheet: string): OverviewMetric {
  ...
  return metricValue
    ...
    : semantic
      ? unavailableMetric(
        ...
        resolved?.note ?? "Tabel Penerimaan → Biomassa belum dapat dipetakan ke skema tujuh pemasok terbaru.",
      )
      : unavailableMetric(
        "ton",
        `Google Sheets ${worksheet} · Penerimaan → Biomassa (7 pemasok)`,
        resolved?.note ?? "Semantic scan tabel Penerimaan → Biomassa tidak tersedia. KPI tidak memakai fallback legacy.",
      );
}

function overviewSeries(semantic: DynamicParserResult | null): OverviewDailyPoint[] | null {
...
function dynamicFocusRecord(semantic: DynamicParserResult | null, query: OverviewQuery) {
  const records = semantic?.normalized.series ?? [];
  if (!records.length) return null;
  const targetDay = query.day ?? new Date().getUTCDate();
  return records.find((record) => record.day === targetDay)
...
function dynamicUnitValues(
  ...
  return record
    ? values.map(([unit, value]) => ({ unit, value }))
    : null;
}

function dynamicHopValues(record: DynamicDailyRecord | null): OverviewHopRow[] | null {
  ...
  return values.map(([unit, value]) => ({ unit, value, ...statusForHop(value as number) }));
}
...
function buildGoogleData(
  ...
  if (rows.length === 0) {
    ...
    return {
      ...
      metrics: {
        biomassReceiptMonthly: unavailableMetric("ton", `${worksheet} · Penerimaan → Biomassa (7 pemasok)`, note),
  ...
  const actualDate = dynamicFocus?.date ?? `${query.year}-${String(query.month).padStart(2, "0")}-${String(actualDay).padStart(2, "0")}`;
  const legacyStockMetric = metric(numericValue(dailyRow, COL.stock), "ton", `Google Sheets ${worksheet} · AD baris harian`);
  ...
  const hopValues = dynamicHop
    ...
    : [numericValue(dailyRow, COL.hop1), numericValue(dailyRow, COL.hop2), numericValue(dailyRow, COL.hop3)];
  const targetRaw = numericValue(targetRow, COL.cumulative);
  const legacyTarget = targetRaw > 0 && targetRaw < 1000 && /^\d{1,3}[.,]\d{3}$/.test(String(targetRow[COL.cumulative] ?? ""))
  ...
  const progress = target !== null && cumulative !== null && target > 0
  ...
  const dynamicBiomassDaily = dynamicFocus && [
    ...
    : null;
  const biomassDaily: OverviewUnitValue[] = dynamicBiomassDaily ?? [
  ...
  const dynamicCoalDaily = dynamicFocus && [
    ...
    : null;
  const coalDaily: OverviewUnitValue[] = dynamicCoalDaily ?? [
  ...
  const biomassReceiptMetric = semanticBiomassReceiptMetric(semantic, worksheet);
  const biomassConsumptionMetric = semanticMetricOrLegacy(
    ...
    metric(numericValue(totalRow, COL.biomassConsumption), "ton", `Google Sheets ${worksheet} · AC42`),
  );
  const coalConsumptionMetric = semanticMetricOrLegacy(
    ...
    metric(numericValue(totalRow, COL.coalConsumption), "ton", `Google Sheets ${worksheet} · AB42`),
  );
  const solarConsumptionMonthlyMetric = semanticMetricOrLegacy(
    ...
    metric(numericValue(totalRow, COL.solarDaily), "liter", `Google Sheets ${worksheet} · CJ42`),
  );
  const solarReceiptMonthlyMetric = semanticMetricOrLegacy(
    ...
    metric(numericValue(totalRow, COL.solarReceipt), "liter", `Google Sheets ${worksheet} · CC42`),
  );
  ...
  const coalReceiptMetric = semanticMetricOrLegacy(
    ...
    metric(numericValue(totalRow, COL.coalReceipt), "ton", `Google Sheets ${worksheet} · I42`),
  );
  const solarConsumptionDailyMetric = semanticMetricOrLegacy(
    ...
    metric(nullableValue(dailyRow, COL.solarDaily) ?? 0, "liter", `Google Sheets ${worksheet} · CJ baris harian`),
  );
  const targetOverview = target !== null && cumulative !== null && progress !== null
    ? { target, cumulative, remaining: Math.max(0, target - cumulative), progress }
    : null;
  ...
  return {
    ...
    source: { label: "Google Sheets API v4", worksheetEquivalent: worksheet, note: "KPI Google Sheets memakai mapping semantic terbaru; KPI biasa memiliki fallback legacy per-field, sedangkan penerimaan Biomassa wajib memakai tujuh pemasok terbaru." },
    metrics: {
      ...
      coalStock: {
        ...stockMetric,
        progressPercent: stockValue === null ? null : Math.min(100, Math.round((stockValue / 70_000) * 100)),
      },
...
export async function getGoogleSheetsOverviewData(query: OverviewQuery): Promise<OverviewData> {
  ...
  try {
    ...
    return buildGoogleData(query, sheet.worksheet, sheet.rows, false, requestedMonthLabel, query, sheet.semantic);
  } catch (error) {
    ...
    for (let index = 0; index < 12; index += 1) {
      ...
      try {
        ...
        return buildGoogleData({ month, year, day: null }, sheet.worksheet, sheet.rows, true, requestedMonthLabel, query, sheet.semantic);
      } catch {
        continue;
      }
    }
