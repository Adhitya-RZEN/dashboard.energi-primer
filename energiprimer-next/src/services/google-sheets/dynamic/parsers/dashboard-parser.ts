...
function chooseResolution(
  ...
  if (!available.length) {
    return (
      ...
      unavailableValue(
        `Anchor ${field} tidak ditemukan atau tidak memiliki nilai valid.`,
      )
    );
  }
  ...
  if (
    second &&
    Math.abs(best.confidence - second.confidence) <= 0.03 &&
...
function resolveField(
  ...
  if (!matches.length)
    return unavailableValue(`Anchor ${field} tidak ditemukan.`);
...
function isCoalCurrentAnchor(anchor: DetectedAnchor) {
  return /PEMAKAIAN\s+(?:BATUBARA|BATU BARA)\s+UNIT\s+[123]\s+(?:CURENT|CURRENT|TERKINI)/.test(
    anchor.matchedLabel,
  );
}
...
function resolveCoalCurrent(
  ...
  const explicit = current.filter(
    (anchor) => explicitUnit(anchor) === requestedUnit,
  );
  ...
  if (!selected)
    return {
      value: unavailableValue(`Anchor ${field} tidak ditemukan.`),
      warning,
    };
  const value = resolveAnchorValue(
    selected,
...
export function parseDashboardTable(
  ...
  const dashboardFields = DASHBOARD_FIELD_DEFINITIONS.filter(
    (definition) => definition.tableKind === "dashboard",
  );

  for (const definition of dashboardFields) {
    if (
      definition.field === "coalUnit1Current" ||
      definition.field === "coalUnit2Current" ||
      definition.field === "coalUnit3Current"
    ) {
      const resolved = resolveCoalCurrent(
        definition.field,
        anchors,
        regions,
        worksheet,
    ...
    fields[definition.field] = resolveField(
      definition.field,
      anchors,
      regions,
      worksheet,
