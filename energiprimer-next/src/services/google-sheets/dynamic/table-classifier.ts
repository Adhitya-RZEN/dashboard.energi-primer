...
export function classifyTable(
  ...
  if (
    hasAny(values, ["TARGET PEMAKAIAN BIOMASSA", "TARGET BIOMASSA"]) ||
    values.some((value) => /^TARGET\s+\d{4}$/.test(value))
  ) {
    return "target";
  }

  if (
    hasAny(values, ["KUMULATIF PEMAKAIAN BIOMASSA"]) ||
    values.some((value) => /^(?:TOTAL|PEMAKAIAN)\s+\d{4}$/.test(value))
  ) {
    return "historical";
  }
