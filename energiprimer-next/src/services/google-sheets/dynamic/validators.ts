...
export function parseNumericValue(raw: unknown): NumericParseResult {
  if (raw === null || raw === undefined) return { value: null, status: "empty", isPercent: false };
  ...
  let text = String(raw)
    ...
    .replace(/[\u00a0\u2007\u202f\s]/g, "")
    .trim();
  if (EMPTY_MARKERS.has(text.toUpperCase())) return { value: null, status: "empty", isPercent: false };
  ...
  if (!/^[+-]?[\d.,]+$/.test(text)) return { value: null, status: "malformed", isPercent };
  ...
  if (!Number.isFinite(value)) return { value: null, status: "malformed", isPercent };
...
export function parseTargetNumber(raw: unknown): NumericParseResult {
  if (typeof raw === "string") {
    ...
    if (/^[+-]?\d{1,3}[.,]\d{3}$/.test(text)) {
      ...
      return { value, status: Number.isFinite(value) ? "numeric" : "malformed", isPercent: false };
    }
  }
...
export function parseDayValue(raw: unknown): number | null {
  ...
  if (typeof raw === "number") return raw >= 1 && raw <= 31 ? Math.trunc(raw) : null;
  const text = String(raw).trim();
  const iso = text.match(/^\d{4}[-/]\d{1,2}[-/](\d{1,2})/);
  if (iso) return Number(iso[1]) >= 1 && Number(iso[1]) <= 31 ? Number(iso[1]) : null;
  const date = text.match(/^(\d{1,2})(?:\s|[-/])/);
  if (!date) return /^\d{1,2}$/.test(text) && Number(text) >= 1 && Number(text) <= 31 ? Number(text) : null;
  const day = Number(date[1]);
  return day >= 1 && day <= 31 ? day : null;
}

export function dateFromRaw(raw: unknown, month: number, year: number): string | null {
  const day = parseDayValue(raw);
  if (day === null) return null;
  if (typeof raw === "string") {
    const iso = raw.trim().match(/^(\d{4})[-/](\d{1,2})[-/](\d{1,2})/);
    if (iso) return `${iso[1]}-${String(Number(iso[2])).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
    const slash = raw.trim().match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{2,4})/);
    if (slash) {
      const fullYear = slash[3].length === 2 ? 2000 + Number(slash[3]) : Number(slash[3]);
...
export function isCompatibleUnit(actual: string | null, expected: readonly string[]): boolean {
...
export function validateUnit(raw: DynamicSheetValue, expected: readonly string[]) {
