...
function distance(a: DetectedAnchor, b: DetectedAnchor) {
  return Math.abs(a.cell.row - b.cell.row) + Math.abs(a.cell.column - b.cell.column);
}

function withinAnchorBand(anchor: DetectedAnchor, group: readonly DetectedAnchor[]) {
...
function expandRowStart(
  ...
  let emptyRows = 0;
  for (let row = initialStart - 1; row >= Math.max(1, initialStart - 30); row -= 1) {
    if (rowHasCellInBand(cells, row, startColumn, endColumn)) {
      start = row;
      emptyRows = 0;
    } else {
...
function columnBand(
  ...
  const nearby = nonEmptyCells(cells).filter((cell) =>
    cell.row >= rowStart - 1
    && cell.row <= rowEnd + 1
  ...
  for (const cell of nearby) frequency.set(cell.column, (frequency.get(cell.column) ?? 0) + 1);
  ...
  const start = Math.min(anchorColumn, ...(columns.length ? columns : [anchorColumn]));
  const end = Math.max(anchorColumn, ...(columns.length ? columns : [anchorColumn]));
  return { start: Math.max(1, start), end };
}

function titleForRegion(cells: readonly ScannedCell[], startRow: number, endRow: number, startColumn: number, endColumn: number) {
  const titleCell = nonEmptyCells(cells)
    ...
    .find((cell) => typeof cell.rawValue === "string" && !/^\d/.test(cell.normalizedValue));
...
function makeRegion(
  ...
  const selected = nonEmptyCells(cells).filter((cell) =>
    cell.row >= startRow
    && cell.row <= endRow
  ...
  return {
    ...
    cells: selected,
    anchors: group,
    title: titleForRegion(selected, startRow, endRow, Math.max(1, band.start - 1), band.end + 1),
    confidence: Math.min(0.99, 0.58 + group.length * 0.04),
  };
}

function findGroups(anchors: readonly DetectedAnchor[], kind: SemanticTableKind) {
  ...
  for (const anchor of remaining) {
    const group = groups.find((candidate) => withinAnchorBand(anchor, candidate));
...
export function detectTableRegions(
  ...
  const dashboardHeader = anchors.filter((anchor) => anchor.key === "dashboardHeader");
  const dashboardGroup = dashboardHeader.length
    ? anchors.filter((anchor) => anchor.tableKind === "dashboard" && withinAnchorBand(anchor, dashboardHeader))
...
export function cellsInRegion(cells: readonly ScannedCell[], region: TableRegion): ScannedCell[] {
...
export function nearestRegion(
  ...
  return candidates.sort((a, b) => {
    const aDistance = Math.abs(anchor.cell.row - (a.startRow + a.endRow) / 2) + Math.abs(anchor.cell.column - (a.startColumn + a.endColumn) / 2);
    const bDistance = Math.abs(anchor.cell.row - (b.startRow + b.endRow) / 2) + Math.abs(anchor.cell.column - (b.startColumn + b.endColumn) / 2);
