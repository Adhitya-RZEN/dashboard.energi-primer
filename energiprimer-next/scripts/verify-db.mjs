...
if (!databaseUrl) {
  console.error(
    "DATABASE_URL belum diatur. Salin .env.example ke .env.local dan isi koneksi existing PostgreSQL.",
  );
  process.exit(2);
}
...
try {
  ...
  const [qualitySummary, qualityEquivalent] = await Promise.all([
    Promise.all([
      prisma.coalQuality.count({
        where: { gar: { gte: new Prisma.Decimal(4700) } },
      }),
      prisma.coalQuality.count({
        where: {
          gar: { gte: new Prisma.Decimal(4500), lt: new Prisma.Decimal(4700) },
        },
      }),
      prisma.coalQuality.count({
        where: { gar: { lt: new Prisma.Decimal(4500) } },
      }),
      prisma.coalQuality.aggregate({
        _count: { _all: true },
        _avg: { gar: true },
      }),
    ]),
  ...
  assertEqual(
    "quality average_gar",
    decimalToNumber(aggregate._avg.gar?.toDecimalPlaces(0) ?? null),
    decimalToNumber(equivalent.average_gar),
  );

  const orphanChecks = await Promise.all([
    prisma.$queryRaw(
      Prisma.sql`SELECT COUNT(*)::int AS count FROM coal_quality cq LEFT JOIN units u ON u.id = cq.unit_id WHERE u.id IS NULL`,
    ),
    prisma.$queryRaw(
      Prisma.sql`SELECT COUNT(*)::int AS count FROM coal_consumption cc LEFT JOIN units u ON u.id = cc.unit_id WHERE u.id IS NULL`,
    ),
    prisma.$queryRaw(
      Prisma.sql`SELECT COUNT(*)::int AS count FROM power_generation pg LEFT JOIN units u ON u.id = pg.unit_id WHERE u.id IS NULL`,
    ),
    prisma.$queryRaw(
      Prisma.sql`SELECT COUNT(*)::int AS count FROM kpi_targets kt LEFT JOIN units u ON u.id = kt.unit_id WHERE u.id IS NULL`,
    ),
  ]);
    ...
    JSON.stringify(
      {
        ...
        qualitySummary: {
          ...
          perhatian,
          offSpec,
          averageGar: decimalToNumber(
            aggregate._avg.gar?.toDecimalPlaces(0) ?? null,
          ),
        },
        checks: [
